...
from mu_engine.providers.catalog import ModelKind, Task, WarmLocalConfig
...
# RELATIVE, on purpose — see ``_slm_support`` module docstring: an absolute
# ``from tests.pipelines...`` import resolves under only ONE of the two module names pytest
# may give this file, and which one it picks depends on ``sys.path``.
