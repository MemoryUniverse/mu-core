...
# `ShippedCatalogSettings` now lives in `shipped_settings.py` and is MOUNTED on
# `ModelCatalogSettings.shipped` (ENG-115a wiring) so `MU_MODEL_CATALOG__SHIPPED__*` reaches it.
# Re-exported here (and in `__all__` above) so every existing import keeps working; the split is
# purely to break the settings.py <-> shipped_catalog.py import cycle the mount would create.
