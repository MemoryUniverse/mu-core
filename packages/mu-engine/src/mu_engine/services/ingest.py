...
from mu_engine.platform.decorators import guard, timed
...
class IngestService:
    ...
    @guard("ingest.remember")
    @timed("ingest.remember")
    async def remember(self, activity: IngestActivity) -> IngestResult:
        ...
        content_hash = str(ctx.state.get("content_hash") or "") or _build_memory_item(
